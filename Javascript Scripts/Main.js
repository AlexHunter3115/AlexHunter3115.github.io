...
function toggleGif(id, src) {
  ...
  if (imageElement.src.includes(src)) {
    const originalSrc = src.includes("gifs")
      ? src.replace("gifs", "pictures").replace(".gif", "_static.png")
      : src.replace("pictures", "gifs").replace("_static.png", ".gif");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function revealSections() {
    for (let section of sections) {
      if (
        isInViewport(section).topInView ||
        isInViewport(section).bottomInView
      ) {
